...
def get_relative_pos(x, bs, grid_size):
    x = x.view(1, -1, 1).expand(bs, -1, -1)
    return  x / grid_size

def get_grids_pos(bs, grid_size=7):
    x = torch.arange(0, grid_size).float().cuda()
    y = torch.arange(0, grid_size).float().cuda()
    px_min = x.view(-1, 1).expand(-1, grid_size).contiguous().view(-1)
    py_min = y.view(1, -1).expand(grid_size, -1).contiguous().view(-1)
    px_max = px_min + 1
    py_max = py_min + 1

    x_min = get_relative_pos(px_min, bs, grid_size)
    y_min = get_relative_pos(py_min, bs, grid_size)
    x_max = get_relative_pos(px_max, bs, grid_size)
    y_max = get_relative_pos(py_max, bs, grid_size)
...
def BoxRelationalEmbedding(f_g, dim_g=64, wave_len=1000, trignometric_embedding=True):
    ...
    x_min, y_min, x_max, y_max = get_grids_pos(batch_size)
