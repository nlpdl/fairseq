...
@dataclass
class EncoderConfig(FairseqDataclass):
    layers: str = field(
        default="[3, 4, 23, 3]",
        metadata={
            "help": "resnet101 structure"
        },
    )
    module_layers: int = field(
        default=6,
        metadata={
            "help": "原本的encoder层数"
        },
    ...
    embed_dim: Optional[int] = field(
        default=2048, metadata={"help": "embedding dimension"}
    )
    ...
    learned_pos: bool = field(
        default=True, metadata={"help": "use learned positional embeddings"}
    )
...
class PretrainConfig(FairseqDataclass):
    ...
    transformer_enc_positional_dropout_rate: float = field(
        default=0.1,
        metadata={"help": "transformer_enc_positional_dropout_rate"},
    )

    input_channel: int = field(
        default=10,
        metadata={"help": "input_channel"},
    )

    output_channel: int = field(
        default=10,
        metadata={"help": "output_channel"},
    )

    freeze_encoder_updates:int = field(
        default=0,
        metadata={"help":"freeze_encoder_updates"}
    )

    no_freeze_encoder_layer:str = field(
        default=None,
        metadata = {"help":"no_freeze_encoder_layer"}
    )

    use_sent_enc_layer:bool = field(
        default=False,
        metadata={"help":"use_sent_enc_layer"}
    )

    layer_norm_first:bool = field(
        default=False,
        metadata={"help":"layer_norm_first"}
    )

    layer_norm_eps:float = field(
        default=1e-5,
        metadata={"help":"layer_norm_eps"}
    )

    relative_position_embedding:bool = field(
        default=False,
        metadata={"help":"relative_position_embedding"}
    )

    latent_dim:int = field(
        default=0,
        metadata={"help":"latent_dim"}
    )
    ...
    latent_temp:str = field(
        ...
        metadata={"help":"(2, 0.5, 0.999995)"}
    )

    latent_groups:int = field(
        default=2,
        metadata={"help":"latent_groups"}
    )

    quantizer_depth:int = field(
        default=1,
        metadata={"help":"quantizer_depth"}
    )

    quantizer_factor:int = field(
        default=3,
        metadata={"help":"quantizer_factor"}
    )

    taskname:str = field(
        default="text_pretrain",
        metadata={"help":"taskname"}
    )
    ...
    remove_contrastive: bool = field(
        default=False,
        metadata={
            "help":"remove_contrastive"
        },
    )
