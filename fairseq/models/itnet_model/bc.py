...
class BCNet(nn.Module):
    ...
    def forward(self, v, q):
        ...
        elif self.h_out <= self.c:#here
            v_ = self.dropout(self.v_net(v)) #V T Y  这里v是Y
            q_ = self.q_net(q)# U T X 这里q是x

            logits = torch.einsum('xhyk,bvk,bqk->bhvq', (self.h_mat, v_, q_)) + self.h_bias # 见公式4中softmax中的内容
    ...
    def forward_with_weights(self, v, q, w):
        ...
        logits = torch.einsum('bvk,bvq,bqk->bk', (v_, w, q_))#见公式5，
        if 1 < self.k:
