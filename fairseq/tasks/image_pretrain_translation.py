...
class TranslationConfig(FairseqDataclass):
    ...
    train_type: str = field(
        default='text_pretrain', metadata={"help": "text_pretrain,img_pretrain,and mix_pretrain,it"}
    )
    label_rates: float = field(
        default=-1, metadata={"help": "if set, looks for labels in this directory instead"}
    )

    sample_rate: float = field(
        default=100, metadata={"help": "target sample rate. audio files will be up/down sampled to this rate"}
    )

    random_crop: bool = field(
        default=False, metadata={"help": "always crop from the beginning if false"}
    )
...
@register_task("image_pretrain_translation", dataclass=TranslationConfig)
class ImagePretrainTranslationTask(FairseqTask):
    """
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        Args:
            split (str):x name of the split (e.g., train, valid, test)
        """
        ...
        if self.cfg.train_type == 'text_pretrain':#只有文本预训练
            ...
            self.datasets[split] = TextOnlyDataset(#只加载文本
                texts,
                source_texts,
        ...
        if self.cfg.train_type == 'img_pretrain':
